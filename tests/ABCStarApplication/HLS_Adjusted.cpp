...
void processHits(ap_uint<16> inputBinaries[MAX_HITS], int inputHitCount, ClusterInfo outputClusters[MAX_CLUSTERS], int& outputClusterCount) {
    ...
    #pragma HLS INTERFACE s_axilite port=inputHitCount bundle=control
    #pragma HLS INTERFACE s_axilite port=outputClusterCount bundle=control
    #pragma HLS INTERFACE m_axi depth=MAX_HITS port=inputBinaries offset=slave bundle=DATA_BUS
    #pragma HLS INTERFACE m_axi depth=MAX_CLUSTERS port=outputClusters offset=slave bundle=DATA_BUS
